...
def _leaf_yolo_model():
    ...
    Set WALK_LEAF_YOLO_MODEL to a custom leaf-trained ultralytics weight file
    when available. Generic COCO YOLO weights do not include a true leaf class,
    so the quality gate below keeps a green-signal fallback for close-up leaves.
    """
    ...
    model_path = os.getenv("WALK_LEAF_YOLO_MODEL", "yolo11n.pt")
...
def _detect_leaf_with_yolo(image: Image.Image) -> tuple[bool, float | None, str | None]:
...
def analyze_walk(
    ...
    skipped_count = sum(
        ...
        or result.status in ("no_leaf_detected", "analysis_failed")
    )

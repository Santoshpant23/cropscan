// Plain-language explanations for every PlantVillage disease class the model
// can return. Interviewees (Yusei, Jaylen) said technical names like
// "corn cercospora leaf spot" meant nothing to them and they had to Google it.
// Keyed by the backend className (must match backend/app/inference.py
// CLASS_NAMES), with a fallback lookup by the friendly display label.
...
function normalizeDiseaseLabel(value: string): string {
  return value
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, ' ')
    .replace(/\s+/g, ' ')
...
const INFO_BY_LABEL: Record<string, DiseaseInfo> = Object.values(
  PLANTVILLAGE_DISEASE_EXPLANATIONS,
).reduce<Record<string, DiseaseInfo>>((lookup, info) => {
  lookup[normalizeDiseaseLabel(info.label)] = info
  return lookup
}, {})

/**
 * Resolve a friendly explanation. Prefers an exact backend className match,
 * then falls back to matching the human display label (e.g. "Late Blight").
 * Returns null when nothing matches so callers can choose to render nothing.
 */
export function getDiseaseInfo(
  ...
  if (disease) {
    const byLabel = INFO_BY_LABEL[normalizeDiseaseLabel(disease)]
    if (byLabel) return byLabel
  }
  return null
}

export { normalizeDiseaseLabel }
